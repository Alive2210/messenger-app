...
public class WebSocketController {
        ...
        /**
         * Handle call invite - sends notification to target user
         */
        @MessageMapping("/call.invite")
        public void handleCallInvite(@Payload CallSignalingDTOs.CallInviteRequest request, Principal principal) {
                log.info("📞 Call invite from {} to {}, video: {}", principal.getName(), request.getTargetUserId(),
                                request.isVideo());
                ...
                log.info("📞 Call invite sent to {}", request.getTargetUserId());
        }

        /**
         * Handle call accept - notifies the caller that the callee accepted
         */
        @MessageMapping("/call.accept")
        public void handleCallAccept(@Payload CallSignalingDTOs.CallAcceptRequest request, Principal principal) {
                log.info("✅ Call accepted by {} for conference {}", principal.getName(), request.getConferenceId());

                CallSignalingDTOs.CallEventNotification notification = CallSignalingDTOs.CallEventNotification.builder()
        ...
        /**
         * Handle call reject - notifies the caller that the callee rejected
         */
        @MessageMapping("/call.reject")
        public void handleCallReject(@Payload CallSignalingDTOs.CallRejectRequest request, Principal principal) {
                log.info("❌ Call rejected by {} for conference {}", principal.getName(), request.getConferenceId());
        ...
        /**
         * Handle call end - notifies all participants
         */
        @MessageMapping("/call.end")
        public void handleCallEnd(@Payload CallSignalingDTOs.CallEndRequest request, Principal principal) {
                log.info("🔚 Call ended by {} for conference {}", principal.getName(), request.getConferenceId());

                // Notify via conference topic so all participants get the event
                messagingTemplate.convertAndSend(
        ...
        private void handleAutomaticVideoRecovery(String conferenceId, Principal principal) {
                ...
                if (frames.length > 0) {
                        ...
                        ReconnectionDTOs.VideoRecoveryResponse response = ReconnectionDTOs.VideoRecoveryResponse
                                        .builder()
                                        .success(true)
        ...
        // Location sharing via WebSocket was removed from this version to keep
        // stability
